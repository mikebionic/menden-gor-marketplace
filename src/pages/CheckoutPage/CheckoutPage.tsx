...
const CheckoutPage: React.FC<ICheckoutPage> = (props: any) => {
  ...
  const handleSubmit = () => {
    ...
    : orderService.checkoutSaleOrderInv([toJsonCheckoutOrderInv(inputs)]).then(
      ...
      (error:any) => sapswal.fire({icon: "error", text: `Failed to checkout order: ${error.toString()}`})
    )
    // : console.log(inputs)
  }
  const handleResponse = (response:any) => {
    ...
    if (response.status === 1){
      console.log("clear cart redux and move to orders page, or just move to main page timeout")
    }
  }
