...
export const WishlistButton: React.FC<IWishlistProps> = ({
  ...
  return (
    <IconLabelButton
      className={`relative bottom-0 right-0 float-right border border-white dark:border-darkComponentColor rounded-md hover:shadow-sm bg-fullwhite dark:bg-darkComponentColor h-9 w-9 ${margin}`}
      onClick={() => {
        setActive(!active);
        onSave(resId, active);
      }}
      icon={
        active ? (
          <HeartFilled className="w-6 h-full mx-auto my-0 text-2xl text-red-500 dark:text-darkFirstColor" />
        ) : (
          <HeartOutlined className="w-6 h-full mx-auto my-0 text-2xl text-red-500 dark:text-darkFirstColor" />
        )
      }
    />
  );
